...
def call_gemini_json(client, model, prompt, max_retries=3):
    ...
    falling back to FALLBACK_MODEL if the primary model keeps failing.
    """
        ...
        for attempt in range(max_retries):
            try:
                ...
                if is_overloaded and not is_last_attempt:
                    wait_time = 2 ** attempt
                    print(f"  [retry] {attempt_model} overloaded, waiting {wait_time}s...")
                    time.sleep(wait_time)
                    continue
                elif is_overloaded:
                    print(f"  [fallback] {attempt_model} still overloaded, trying fallback model...")
                    break
                else:
                    raise
